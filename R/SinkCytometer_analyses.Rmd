...
SinkMeta <- MetaData %>%
  ...
  dplyr::select(-c(id, well, well_target, PropidiumIod_vol_uL)) %>% #unselect Propidium iodide here, as there was a pipetting error once! Calculate true cellcount before that step!!!
  mutate(exp_date = ymd(exp_date))
  
SinkCC <- ImportSink %>%
  dplyr::select(strain, date_sink, datetime, par_ue, StartMin, ElapSec, ElapMin, TimeID, well, well_target, CorrDeadCells, AliveCells, DeadAliveCells, TotCy5Cells, culture_vol_uL, PropidiumIod_vol_uL) %>%
  full_join(SinkMeta) %>%
  distinct() %>%
  ...
  drop_na(project_id) %>%
  mutate(
         ShareAliveTotCy = max(AliveCells)/max(TotCy5Cells)*100,
         ShareDeadAliveTotCy = max(DeadAliveCells)/max(TotCy5Cells)*100,
         sink_hour = datetime + seconds(ElapSec),
...
SinkBreak <- SinkCC %>%
  ...
  dplyr::select(GroupID, Cellcount_L, sink_hour) %>%
  group_by(GroupID) %>%
  nest() %>%
  mutate(ts = map(.x = data, 
         ...
         opt_brks = purrr::modify(opt_brks, ~ if(length(.) == 0){  #https://stackoverflow.com/questions/67073645/replace-numeric0-with-0-in-r-list
                                   0 
                                   } else { . })) %>%
  unnest_wider(opt_brks, names_sep = "_", names_repair = "unique") %>%
  rename("opt_brks" = "opt_brks_1") %>%
...
df_strain <- SinkCC %>%
  ungroup() %>%
  dplyr::select(Viability, GroupID, strain, date_sink, par_ue) %>%
  distinct()
...
Brk_Compare <- full_join(SinkCC, impute_brkpt) %>%
  ...
  mutate(BrkTime = case_when(BrkptNo != "NA" ~ ElapMin*1),
         BrkTime = replace(BrkTime, is.na(BrkTime), NA),
         BrkptNo = as.numeric(BrkptNo)) %>%
  fill(BrkptNo, .direction = "up") %>%
  fill(BrkTime, .direction = "up") %>%
  mutate(Cellcount = replace(Cellcount, is.na(Cellcount), 0), # replace NA with 0, as nothing was detected
         ...
         CellcountDiff = coalesce(CellcountDiff, Cellcount), #replaces NAs in CellcountDiff for TimeID1 with the cellcount at this time point, TimeID0 would be 0 cells, but is not present so causes NA instead
  ...
  mutate(CellcountDiffSD = sd(CellcountDiff, na.rm = TRUE),  
         ...
         PeakTime = if_else(CellcountMax > 0, ElapMin, 0), #peak time calculated for all cellcount, also calculated for CellCountMax > 0 ? 
         ...
         sink_vel_mm_min = sink_height_mm/PeakTime,
         relPeakOcc = PeakTime/BrkTime
         ) 
...
BrkSink <- Brk_Compare %>%
  ...
  drop_na(BrkptNo) %>%
  #        #BrkptNo == "Brkpt_1") %>%
  ggplot(aes(y = log(CellcountMax), x = ElapMin, color = as.factor(par_ue))) +
  geom_point() +
  ...
  #geom_vline(aes(xintercept = PeakTime, color = as.factor(BrkptNo))) +
  facet_grid(
     rows = vars(Viability),
    cols = vars(date_sink)
    ) +
...
SinkSizeTotArea <- ImportSink %>%
  dplyr::select(date_sink, TimeID, well_target, well, strain, par_ue, Cy5TotArea, DeadTotArea, DeadAliveCTotArea) %>%
...
SinkSize <- ImportSink %>%
  dplyr::select(date_sink, TimeID, well_target, well, strain, par_ue, Cy5AvArea, DeadAvArea, DeadAliveCAvArea) %>%
  ...
  mutate(CellcountDiff = replace(CellcountDiff, is.na(CellcountDiff), 0),
         ...
         AvCellSurfDiff = (TotCellArea - lag(TotCellArea))/(CellcountDiff),  #create change in cell surface area between time points
         ...
         CellDiameter = (2*(sqrt(AvCellSurfDiff/pi)))
         ) %>%
...
SinkCC %>% 
  ...
  #drop_na(BrkptNo) #%>%
  #        #BrkptNo == "Brkpt_1") %>%
  ggplot(aes(y = (Cellcount), x = (ElapMin), color = as.factor(par_ue))) +
  geom_point() +
...
SinkSizePlot <- Brk_Compare %>% 
  group_by(par_ue, strain, date_sink, Viability) %>%
  filter(
         #strain %in%c("CCMP836", "CZS25K", "PCC6803"),
         strain %in%c(#"CCMP495",
                      "CCMP1335"
                      #"CCMP1336"
                      ),
         ...
         BrkptNo == 1,
         #par_ue == 30,
         #date_sink == "2022-10-21",
         #date_sink == c("2022-10-15", "2022-10-17", "2022-10-19", "2022-10-21"),
         #well == 'A2'
         #CellAreaScale >= 0,
         #CellAreaScale <= 1,
  ...
  #mutate(sink_vel_mm_min = sink_vel_mm_min[which.max(CellcountStep)]) %>% #selects only sinking rates per well that coincide with cell count maximum
  #drop_na(BrkptNo) #%>%
  #        #BrkptNo == "Brkpt_1") %>%
  ggplot(aes(y = (sink_vel_mm_min), x = as.factor(date_sink), color = as.factor(par_ue))) +
  #geom_point() +
 ...
 #                      )) +
  ...
  #             #,  formula = y ~ 0 + x, se=TRUE, color = "black"
  #             ) +
  facet_grid(
     rows = vars(Viability),
    cols = vars(par_ue)
...
ggsave(filename = file.path(paste(FP1, FP1, PlotFolder, sep = "/"), paste(Project, "Diatom", "SinkVeloPlot.png", sep = "_"), fsep = .Platform$file.sep), plot = SinkSizePlot, height = 10, width = 12)
...
ImportSink %>%
  filter(  
         strain == "PCC6803", #PCC6803 CZS25K CCMP836 CCMP1336 CCMP1335 CCMP495
         #date_sink != "2022-10-14"
         #Cy5AvArea < 7,
         ElapMin < 200
  ) %>%
  ggplot(aes(y = (Cy5AvArea), x = (ElapMin), color = as.factor(par_ue))) + #https://support.moleculardevices.com/s/article/MetaXpress-and-MetaMorph-Relationship-between-Average-Intensity-and-Integrated-Intensity
  geom_point() +
  geom_smooth(method = "loess"
               #,  formula = y ~ 0 + x, se=TRUE, color = "black"
                ) +
...
#calculate a GAM model based on Cy5AvArea ~ ElapMin with log-scaled normalized data points and sample_date as random factor. See if signal of smaller cells in colonies, larger single cells and smaller single cells persists. 

SinkSize %>%
  filter(  
         strain == "CCMP1336", #PCC6803 CZS25K CCMP836 CCMP1336 CCMP1335 CCMP495
         #date_sink != "2022-10-14"
         #Cy5AvArea < 7,
         ElapMin < 300,
         AvCellArea < 1000
  ) %>%
  ggplot(aes(y = (AvCellArea), x = (ElapMin)
             , color = as.factor(Viability)
             )) + #https://support.moleculardevices.com/s/article/MetaXpress-and-MetaMorph-Relationship-between-Average-Intensity-and-Integrated-Intensity
  geom_point() +
  geom_smooth(method = "loess"
               #,  formula = y ~ 0 + x, se=TRUE, color = "black"
                ) +
...
GrowthCC %>%
  filter(strain %in%c("CCMP836", "CZS25K", "PCC6803"),
         #strain %in%c("CCMP495", "CCMP1335", "CCMP1336")
         ) %>%
  filter(Viability %in% c("TotCy5Cells", "CorrDeadCells"),
         Cellcount_L < 200000000000) %>%
  #filter(exp_date == "20200902") %>%
  #filter(tissue_treated_plate == TissueTreated) %>%
  #filter(media == Media) %>%
  #filter(SourceSal == source_salinity) %>%
  ggplot(aes(x = E_days, y = (!!sym(FitVar)),  color = as.factor(!!sym(ColVar2))))+
  geom_point() +
  #geom_line(aes(x = E_days, y = log(!!sym(FitVar)), color = as.factor(!!sym(ColVar2)))) +
  geom_smooth(method = "loess") +
...
CellLogSpecificPlot <- CellLogSpecificPredict %>% 
  dplyr::filter(Viability == "TotCy5Cells",
                strain %in% c("CCMP495", "CCMP1335", "CCMP1336")) %>%
  ggplot() +  
  geom_line(aes(x = E_days, y = log10(.fitted), color = as.factor(well)), size = 0.5) +
  geom_point(aes(x = E_days, y = log10(!!sym(FitVar)), color = as.factor(well)), size = 0.6) +
...
CellLogSpecificCoeff %>% 
  ...
  geom_point(aes(x = par_ue, y = estimate, color = as.factor(par_ue)), size = 1) +
  # geom_ribbon(aes(x = E_days, ymin = (.fitted - .resid), ymax = (.fitted + .resid), alpha = 0.1),show.legend = FALSE) +
...
SelectStrain <- 'CCMP495' #"CCMP495", "CCMP1335", "CCMP1336" "CCMP836", "CZS25K", "PCC6803"
SelectViability <- "DeadAliveCells" #CorrDeadCells AliveCells DeadAliveCells TotCy5Cells

# ExportCellCount <- GrowthCC %>%
#   select(date_sink, well, Viability, par_ue, strain, Cellcount_L) %>%
#   dplyr::filter(
#          strain %in% SelectStrain,
#          Viability %in%  SelectViability #CorrDeadCells AliveCells DeadAliveCells TotCy5Cells
#          ) %>%
#   pivot_wider(names_from = par_ue, names_prefix = "Cell_L_par", names_sep = "_", values_from = c(Cellcount_L))
# 
# 
# h5write(ExportCellCount, "ExportCellCount.h5", name = paste(SelectViability, SelectStrain))

```



``` {r growth rate based on Chlorophyll extraction, warning = FALSE} 
#grouping variables for rows & columns, and for nesting for fitting
RowVar <- "par_ue" 
RowVar2 <- "date_sink"
ColVar <- "strain"
...
test <- full_join(ChlLogSpecificCoeff %>% rename("Chl_estimate" = "estimate") %>% dplyr::select(-c(std.error, statistic, p.value)), CellLogSpecificCoeff %>% rename("Cell_estimate" = "estimate") %>% filter(Viability %in% c("AliveCells")) %>% dplyr::select(-c(std.error, statistic, p.value, Viability))) %>%
  dplyr::filter(term %in% c("Mu")) %>%
...
# MaxSinkCC <- SinkCC %>% 
#   mutate(MaxCellcount_L == max(Cellcount_L, na.rm = TRUE)) 

Sink_Chl_Cell <- ChlFluor %>%
  mutate(exp_date = ymd(exp_date))%>%
  full_join(SinkCC %>% mutate(MaxCellcount_L = max(Cellcount_L, na.rm = TRUE))) %>%
  mutate(
         Chl_ugL_corr = case_when(Viability == "AliveCells" ~ Chl_ugL_mean*(ShareAliveTotCy/100),
                                  Viability == "DeadAliveCells" ~ Chl_ugL_mean*(ShareDeadAliveTotCy/100),
                                  #Viability == "TotCy5Cells" ~ Chl_ugL_mean, #sum of alive + DeadAlive, normal Chl-signal sufficient
                                  #Viability == "CorrDeadCells" ~ Chl_ugL_mean*0 #don't possess Chlorophyll signal
                                  ),
         pg_Chl_Cell = case_when(Viability == "TotCy5Cells" ~ Chl_ugL_mean/MaxCellcount_L*10^6,
                                 Viability == "CorrDeadCells" ~ Chl_ugL_mean*0,
                                 Viability == "AliveCells" ~ Chl_ugL_corr/MaxCellcount_L*10^6, #use corrected Chl-amount on respective maximum cell count found that day in living cells based on living:DeadAlive
                                 Viability == "DeadAliveCells" ~ Chl_ugL_corr/MaxCellcount_L*10^6) #use corrected Chl-amount on respective maximum cell count found that day in living cells based on living:DeadAlive
         ) %>%
  drop_na(pg_Chl_Cell) %>%
  dplyr::select(date_sink, ElapMin, well, Viability, par_ue, strain, sink_hour, pg_Chl_Cell) %>%
  full_join(Brk_Compare) %>%
  mutate(
         #pg_Chl_Cell = Chl_ugL_mean/Cellcount_L*10^6,
         #pg_Chl_Cell = case_when(Viability == "TotCy5Cells" ~ Chl_ugL_mean/Cellcount_L*10^6),
         date_sink = (as.character(date_sink)),
         sink_hour = as.character(sink_hour)
         ) %>%
  dplyr::filter(BrkptNo == 1,
         sink_vel_mm_min != Inf,
         CellcountMax > 1,
         CellcountDiffSDFlag == 1
         ) %>% #filter only for significant peaks and maximum cells > 1
  dplyr::select(date_sink, ElapMin, sink_vel_mm_min, well, Viability, par_ue, strain, sink_hour, pg_Chl_Cell, BrkTime, PeakTime, relPeakOcc) 


SelectStrain <- 'PCC6803' #"CCMP495", "CCMP1335", "CCMP1336" "CCMP836", "CZS25K", "PCC6803"
SelectViability <- "TotCy5Cells" #CorrDeadCells AliveCells DeadAliveCells TotCy5Cells

# ExportSink <- Sink_Chl_Cell %>%
#   dplyr::filter(
#          strain %in% SelectStrain,
#          Viability %in%  SelectViability #CorrDeadCells AliveCells DeadAliveCells TotCy5Cells
#          ) %>%
#   pivot_wider(names_from = par_ue, names_prefix = "par", names_sep = "_", values_from = c(sink_vel_mm_min, pg_Chl_Cell))
# 
# 
# h5write(ExportSink, "ExportSinkVelo.h5", name = paste(SelectViability, SelectStrain))
...
possibKMEAN = possibly(.f = kmeans, otherwise = NULL)

ClusterTime <- Brk_Compare %>%
  mutate(PhytoGroup = case_when(strain %in% c("CZS25K","PCC6803","CCMP836") ~ "Cyanobacteria",
                                strain %in% c("CCMP495","CCMP1335","CCMP1336") ~ "Diatoms")) %>%
  dplyr::filter(BrkptNo == 1,
         sink_vel_mm_min != Inf,
         CellcountMax > 1,
         CellcountDiffSDFlag == 1
  ...
  ungroup() %>%
  #group_by(PhytoGroup, par_ue, Viability) %>%
  dplyr::select(date_sink)

############### determination optimum k 
kOpt <- Brk_Compare %>%
  mutate(PhytoGroup = case_when(strain %in% c("CZS25K","PCC6803","CCMP836") ~ "Cyanobacteria",
                                strain %in% c("CCMP495","CCMP1335","CCMP1336") ~ "Diatoms")) %>%
  dplyr::filter(BrkptNo == 1,
  ...
  ungroup() %>%
  group_by(PhytoGroup, par_ue, Viability) %>%
  #mutate_at(c("sink_vel_mm_min"), ~(scale(.) %>% as.vector(.))) %>%
  arrange(sink_vel_mm_min) %>%
  dplyr::select(sink_vel_mm_min, Viability, par_ue, PhytoGroup) 

point <- kOpt %>% 
   ungroup() %>%
   dplyr::select(sink_vel_mm_min)

kclusts <- 
  tibble(k = 1:9) %>%
  mutate(
    kclust = map(k, ~kmeans(point, .x)),
    tidied = map(kclust, tidy),
    glanced = map(kclust, glance),
    augmented = map(kclust, augment, point)
  )
clusterings <- 
  kclusts %>%
...
ggplot(clusterings, aes(k, tot.withinss)) +
  geom_line() +
  geom_point()
# "knee" of the optimum amount of k is ~2.5 so more than 3 centers have little value based on total within sum of squares
################

SinkCluster <- Brk_Compare %>%
  mutate(PhytoGroup = case_when(strain %in% c("CZS25K", "CCMP836", "PCC6803") ~ "cyanobacteria",
                                             strain %in% c("CCMP495", "CCMP1335") ~ "diatoms (<10 µm)",
                                             strain %in% c("CCMP1336") ~ "diatom (> 10 µm)")
           # case_when(strain %in% c("CZS25K","PCC6803","CCMP836") ~ "Cyanobacteria",
           #                      strain %in% c("CCMP495","CCMP1335","CCMP1336") ~ "Diatoms")
         ) %>%
  dplyr::filter(BrkptNo == 1,
         sink_vel_mm_min != Inf,
  ...
  ungroup() %>%
  group_by(date_sink, PhytoGroup, par_ue, Viability) %>%
  #mutate_at(c("sink_vel_mm_min"), ~(scale(.) %>% as.vector(.))) %>%
  arrange(sink_vel_mm_min) %>%
  dplyr::select(sink_vel_mm_min, Viability, par_ue, PhytoGroup) %>%
  nest(sink_vel_mm_min) %>%
  mutate(
    kclust = map(data, ~possibKMEAN(.x[[1]], centers = 2, iter.max = 100, nstart = 100)),
    tidied = map(kclust, tidy),
    glanced = map(kclust, glance),
    ClusterGroup = map(kclust, ~pluck(.x$cluster))
  ) %>%
  #unnest(tidied) %>%
  unnest(ClusterGroup, data) 

ClusterNames <- SinkCluster %>%
  unnest(tidied) %>%
  filter(cluster == ClusterGroup) %>%
  rename("kMean" = "x1") %>%
  dplyr::select(-c(size, withinss, kclust, glanced, sink_vel_mm_min)) %>%
  distinct() %>%
  pivot_wider(id_cols = c(date_sink:PhytoGroup), values_from = "kMean", names_from = cluster, names_prefix =  "Cluster_") %>%
  mutate(Sub_Pop = case_when(Cluster_1 < Cluster_2 ~ "1_slow",
                              Cluster_1 > Cluster_2 ~ "1_fast")) %>%
  dplyr::select(-c(Cluster_2, Cluster_1))

SinkPopulation <- full_join(SinkCluster, ClusterNames) %>%
  mutate(Sub_Pop2 = case_when(Sub_Pop == "1_fast" ~ "2_slow",
                              Sub_Pop == "1_slow" ~ "2_fast")) %>%
  pivot_longer(cols = c(Sub_Pop, Sub_Pop2), names_to = "Population", values_to = "SinkVelo") %>%
  separate(SinkVelo, into = c("Cluster", "SinkVelo"), sep = "_", remove = TRUE) %>%
  dplyr::filter(Cluster == ClusterGroup) %>%
  dplyr::select(c(date_sink:sink_vel_mm_min, SinkVelo))
 
PhytoGroupPlot <- "diatoms (<10 µm)" #cyanobacteria 'diatoms (<10 µm)' 'diatoms (>10 µm)' 

SinClusterPlot <- SinkPopulation %>%
  #cbind(ClusterTime) %>%
  #filter()
  dplyr::filter(PhytoGroup %in% PhytoGroupPlot) %>% 
  mutate(par_ue_f =as.factor(par_ue)) %>%
  ggplot(aes(x = date_sink, y = sink_vel_mm_min, colour = as.factor(SinkVelo), group = Viability), size = 0.5, show.legend = T) +
  #geom_line() +
  #geom_ribbon(aes(y = NULL, ymin = low, ymax = high, fill = Viability, group = Viability), linetype = "dashed", alpha = 0.1, show.legend = F) + 
  geom_point(aes(x = date_sink, y = sink_vel_mm_min, colour = as.factor(SinkVelo), group = Viability), alpha = 0.1, show.legend = T) +
  xlab("Elapsed day") +
  scale_y_continuous(name = "Sinking velocity (mm/min)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  #coord_cartesian(ylim = c(-2.5, 2.5)) +
  facet_grid(cols = vars(Viability), rows = vars(fct_rev(par_ue_f)), scales = "free") +
  # scale_fill_manual(values = Colours_Group, labels=c("Alive cells", "Dead cells", "Dead Alive cells")) +    
  # scale_color_manual(values = Colours_Group, labels=c("Alive cells", "Dead cells", "Dead Alive cells")) + 
  theme_classic()
SinClusterPlot


ggsave(filename = file.path(paste(FP1, FP1, PlotFolder, sep = "/"), paste(Project,PhytoGroupPlot, "Cluster", "SinkPlot.png", sep = "_"), fsep = .Platform$file.sep), plot = SinClusterPlot, height = 10, width = 12)

SubPop <- Brk_Compare %>%
  group_by(Viability, par_ue, strain, well) %>%
  ...
  ungroup() %>%
  full_join(SinkPopulation) %>%
  drop_na(SinkVelo) %>%
  mutate(date_sink = (as.character(date_sink))) %>%
  dplyr::select(date_sink, ElapMin, sink_vel_mm_min, well, Viability, par_ue, strain, sink_hour, SinkVelo)

SelectStrain <- 'PCC6803' #"CCMP495", "CCMP1335", "CCMP1336" "CCMP836", "CZS25K", "PCC6803"
SelectViability <- "CorrDeadCells" #CorrDeadCells AliveCells DeadAliveCells TotCy5Cells

# ExportCluster <- SubPop %>%
#   dplyr::filter(
#          strain %in% SelectStrain,
#          Viability %in%  SelectViability #CorrDeadCells AliveCells DeadAliveCells TotCy5Cells
#          ) %>%
#   pivot_wider(names_from = par_ue, names_prefix = "par", names_sep = "_", values_from = c(sink_vel_mm_min, SinkVelo))
# 
# 
# h5write(ExportCluster, "ExportClusterVelo.h5", name = paste(SelectViability, SelectStrain))

SubPop_AvStrain <- SubPop %>%
  dplyr::filter(Viability != "TotCy5Cells") %>%
  dplyr::mutate(Viability_group = case_when(Viability %in% c("CorrDeadCells", "DeadAliveCells") ~ "Dead&Dying",
                                            Viability %in% c("AliveCells") ~ "Alive")) %>%
  group_by(Viability_group, strain, par_ue, SinkVelo) %>%
  summarise(AvSinkVel_mm_min = mean(sink_vel_mm_min), StdSinkVel_mm_min = sd(sink_vel_mm_min)) 

#h5write(SubPop_AvStrain, "ExportMeanVelo.h5", name = paste("MeanSinkVelo_strain"))

SubPop_AvGroup <- SubPop %>%
  dplyr::filter(Viability != "TotCy5Cells") %>%
  dplyr::mutate(Viability_group = case_when(Viability %in% c("CorrDeadCells", "DeadAliveCells") ~ "Dead&Dying",
                                            Viability %in% c("AliveCells") ~ "Alive"),
                Functional_group = case_when(strain %in% c("CZS25K", "CCMP836", "PCC6803") ~ "picocyanobacteria",
                                             strain %in% c("CCMP495", "CCMP1335") ~ "nanodiatoms",
                                             strain %in% c("CCMP1336") ~ "microdiatom")
                ) %>%
  group_by(Viability_group, Functional_group, par_ue, SinkVelo) %>%
  summarise(GroupSinkVel_mm_min = mean(sink_vel_mm_min), StdSinkVel_mm_min = sd(sink_vel_mm_min)) 
#h5write(SubPop_AvGroup, "ExportMeanVelo.h5", name = paste("MeanSinkVelo_group"))

SubPop_separate <- Brk_Compare %>%
  dplyr::filter(Viability != "TotCy5Cells") %>%
  dplyr::mutate(Viability_group = case_when(Viability %in% c("CorrDeadCells", "DeadAliveCells") ~ "Dead&Dying",
                                            Viability %in% c("AliveCells") ~ "Alive")
                ) %>%
  ...
  drop_na(SinkVelo) %>%
  mutate(date_sink = (as.character(date_sink))) %>%
  dplyr::select(date_sink, ElapMin, sink_vel_mm_min, well, Viability, par_ue, strain, SinkVelo, Viability_group)

SelectStrain <- 'PCC6803' #"CCMP495", "CCMP1335", "CCMP1336" "CCMP836", "CZS25K", "PCC6803"
SelectViability <- "Dead&Dying" #Alive Dead&Dying
SelectVelo <- 'slow' #slow

ExportCluster <- SubPop_separate %>%
  dplyr::filter(
         strain %in% SelectStrain,
         Viability_group %in%  SelectViability,
         SinkVelo %in% SelectVelo#CorrDeadCells AliveCells DeadAliveCells TotCy5Cells
         ) %>%
  pivot_wider(names_from = c(par_ue, SinkVelo), names_prefix = "par", names_sep = "_", values_from = c(sink_vel_mm_min))


#h5write(ExportCluster, "ExportClusterBoxplot.h5", name = paste(SelectViability, SelectStrain, SelectVelo))
...
Brk_GAM <- Brk_Compare %>%
  ...
  ungroup() %>%
  full_join(SinkPopulation) %>%
  drop_na(SinkVelo) #drop NAs in SinkVelo as those sink velocities had too few data points to be clustered, mostly dying cells

E_days_pred <- expand_grid(
  E_days = seq(from=min(0), 
              to=max(7), 
              length.out = 8)
)
...
# steps included to prohibit negative cell count predictions
possibGAM = possibly(.f = gam, otherwise = NULL)
...
#   nest() %>%
#   mutate(
#     GamModel = map(data, ~possibGAM(sink_vel_mm_min ~ s(E_days, k = 4), data = .x, method = "REML")),
#     predicted = map(GamModel, ~possibPRED(.x, newdata = E_days_pred)),
...
#   mutate(E_days_Model = seq(from=min(0), 
#               to=max(7), 
#               length.out = 8)
#          )
# 
# GamDiffPred %>%
#   filter(Viability != "TotCy5Cells")  %>%
#   ggplot(aes(E_days_Model, (predicted))) +
#   geom_line(aes(colour = as.factor(Viability))) +
#   facet_grid(cols = vars(strain), rows = vars(par_ue)) +
#   xlim(1, 8) +
#   theme_bw()
...
GamDiff <- Brk_GAM %>%
  ...
  dplyr::mutate(Viability_group = case_when(Viability %in% c("CorrDeadCells", "DeadAliveCells") ~ "Dead&Dying",
                                            Viability %in% c("AliveCells") ~ "Alive"),
                Functional_group = case_when(strain %in% c("CZS25K", "CCMP836", "PCC6803") ~ "cyanobacteria",
                                             strain %in% c("CCMP495", "CCMP1335") ~ "diatoms (<10 µm)",
                                             strain %in% c("CCMP1336") ~ "diatom (> 10 µm)")
                ) %>%
  group_by(Viability_group, par_ue, Functional_group, SinkVelo) %>%
  nest() %>%
  mutate(
    GamModel = map(data, ~possibGAM(sink_vel_mm_min ~ s(E_days, k = 4), data = .x, method = "REML")),
    predicted = map(GamModel, ~possibPRED(.x, newdata = E_days_pred)),
...
GamDiffPred %>%
  #filter(Viability != "TotCy5Cells")  %>%
  ggplot(aes(E_days_Model, (predicted))) +
  geom_line(aes(colour = as.factor(Viability_group))) +
  facet_grid(cols = vars(Functional_group), rows = vars(par_ue)) +
  xlim(1, 8) +
  theme_bw()
...
Colours_Group = c(fast_Alive = "#CC0000", slow_Alive = "#FF3300", 'fast_Dead&Dying' = "#0072B2", 'slow_Dead&Dying' = "#00CCFF"
                  #, fast_DeadAliveCells = "#CC6600", slow_DeadAliveCells = "#E69F00"
                  ) #https://stackoverflow.com/questions/17180115/manually-setting-group-colors-for-ggplot2

XAxisGrowthLabel <- c("early exp", "mid exp", "late exp", "stationary")

GamSinkPlot <- GamFitPred %>%
  unite("Cluster_Viability", c('SinkVelo','Viability_group'), sep = "_", remove = FALSE) %>%
  # filter(Viability != "TotCy5Cells"
  #        #, SinkVelo == "fast"
  #        ) %>%
  mutate(low = .fitted-.se.fit,
         high = .fitted+.se.fit,
         par_ue_f =as.factor(par_ue)) %>%
  ggplot(aes(x = E_days, y = .fitted, colour = Cluster_Viability,  group = Cluster_Viability), size = 0.5, show.legend = F) +
  geom_line() +
  geom_ribbon(aes(y = NULL, ymin = low, ymax = high, fill = Cluster_Viability, group = Cluster_Viability), linetype = "dashed", alpha = 0.1, show.legend = F) + 
  geom_point(aes(x = E_days, y = sink_vel_mm_min, colour = Cluster_Viability,  group = Cluster_Viability), position = "jitter", alpha = 0.1, show.legend = F) +
  #xlab("Elapsed day") +
  scale_y_continuous(name = bquote(bold("Sinking velocity"~"(mm"~min^-1~")")), 
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  #coord_cartesian(ylim = c(0, 2)) +
  facet_grid(cols = vars(Functional_group), rows = vars(fct_rev(par_ue_f)), scales = "free") +
  scale_fill_manual(values = Colours_Group, labels=c("fast&alive", "fast&dead", "slow&alive",  "slow&dead"
                                                     #, "fast&dying", "slow&dying"
                                                     )) +    
  scale_color_manual(values = Colours_Group, labels=c("fast&alive", "fast&dead", "slow&alive",  "slow&dead"
                                                      #, "fast&dying", "slow&dying"
                                                      )) + 
  scale_x_continuous(breaks = c(1, 3, 5, 7),
                     #, labels = XAxisGrowthLabel
                     name = "growth day") +
  guides(col = guide_legend("Cluster&Viability")) +
  theme_bw() +
  theme(text = element_text(size = 8, face = "bold", color = "black"), strip.background = element_rect(fill="white"), legend.position = "bottom", axis.title.y = element_text(size = 8, face = "bold", color = "black"), axis.text.x = element_text(#angle = 45,
                           #hjust = 1,
                           color = "black"), 
                           axis.text.y = element_text(color = "black"))
GamSinkPlot

ggsave(filename = file.path(paste(FP1, FP1, PlotFolder, sep = "/"), paste(Project, "GAM", "SinkPlot.png", sep = "_"), fsep = .Platform$file.sep), plot = GamSinkPlot, height = 11, width = 11, units = "cm", dpi = 900)

ggsave(filename = file.path(paste(FP1, FP1, PlotFolder, sep = "/"), paste(Project, "GAM", "SinkPlot.tiff", sep = "_"), fsep = .Platform$file.sep), plot = GamSinkPlot, height = 11, width = 11, units = "cm", dpi = 900)

# GamSinkPlotSlow <- GamFitPred %>%
#   filter(Viability != "TotCy5Cells",
#          SinkVelo == "slow") %>%
#   mutate(low = .fitted-.se.fit,
#          high = .fitted+.se.fit,
#          par_ue_f =as.factor(par_ue)) %>%
#   ggplot(aes(x = E_days, y = .fitted, colour = Viability, shape = SinkVelo, group = Viability), size = 0.5, show.legend = F) +
#   geom_line() +
#   geom_ribbon(aes(y = NULL, ymin = low, ymax = high, fill = Viability, group = Viability), linetype = "dashed", alpha = 0.1, show.legend = F) + 
#   geom_point(aes(x = E_days, y = sink_vel_mm_min, colour = Viability, shape = SinkVelo, group = Viability), alpha = 0.1, show.legend = F) +
#   xlab("Elapsed day") +
#   scale_y_continuous(name = "Sinking velocity (mm/min)",
#                      sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
#   #coord_cartesian(ylim = c(0, 0.5)) +
#   facet_grid(cols = vars(PhytoGroup), rows = vars(fct_rev(par_ue_f)), scales = "free") +
#   scale_fill_manual(values = Colours_Group, labels=c("Alive cells", "Dead cells", "Dead Alive cells")) +    
#   scale_color_manual(values = Colours_Group, labels=c("Alive cells", "Dead cells", "Dead Alive cells")) + 
#   theme_classic()
# GamSinkPlotSlow
# 
# p1 <- grid.arrange(grobs= list(GamSinkPlot, GamSinkPlotSlow),
#              ncol=1, nrow=2)
# p1
...
ElapMin_Model <- expand_grid(
  ElapMin = seq(from=min(1), 
              to=max(600), 
              length.out = 600)
)
...
GamSize <- SinkSize %>%
  ...
  nest() %>%
  mutate(
    GamModel = map(data, ~possibGAM(AvCellArea ~ s(ElapMin, k = 20), data = .x, 
                                         family = "gaussian"(link = "log"),
                                         method = "REML")),
    predicted = map(GamModel, ~possibPRED(.x, newdata = ElapMin_Model, type = "response", se.fit = T)),
    augmented = map(GamModel, ~augment(.x)),
    glance = map(GamModel, ~glance(.x))
  )

GamSize_Pred <- GamSize %>%
  dplyr::select(strain, par_ue, Viability, predicted) %>%
  dplyr::filter(predicted != "NULL") %>%
  rowwise() %>%
  mutate(db = list(cbind(predicted, ElapMin_Model))) %>%
  dplyr::select(-predicted) %>%
  unnest(col = db) %>%
  full_join(SinkSize %>% mutate(ElapMin = round(ElapMin))) %>%
  dplyr::filter(Viability != "TotCy5Cells",
                Viability != "DeadAliveCells")  %>%
  mutate(low = fit-2*se.fit,
         high = fit+2*se.fit,
         par_ue_f =as.factor(par_ue)) 

legend_title <- "Viability"
SizeColors <- c("#FF0000", "#0000FF")

GamSize_CCMP836 <- GamSize_Pred %>%
  dplyr::filter(strain == "CCMP836") %>%
  ggplot(aes(ElapMin, fit)) +
  geom_line(aes(colour = as.factor(Viability)), show.legend = F) +
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = ElapMin, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("A - CCMP836"~"("~1.3~ µm~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
  facet_grid(rows = vars(fct_rev(par_ue_f)), scales = "free") +
  coord_cartesian(ylim = c(0, NA)) +
  theme_bw() +
  theme(text = element_text(size = 10, face = "bold", color = "black"), plot.title = element_text(size = 10, face = "bold", color = "black"), strip.background = element_rect(fill="white"), legend.position = "bottom", axis.title.x = element_blank(), axis.text.x = element_text(color = "black"), axis.text.y = element_text(color = "black"))
GamSize_CCMP836

GamSize_CZS25K <- GamSize_Pred %>%
  dplyr::filter(strain == "CZS25K") %>%
  ggplot(aes(ElapMin, fit)) +
  geom_line(aes(colour = as.factor(Viability)), show.legend = F) +
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = ElapMin, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("B - CZS25K"~"("~1.5~ µm~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
GamSize_PCC6803 <- GamSize_Pred %>%
  dplyr::filter(strain == "PCC6803") %>%
  ggplot(aes(ElapMin, fit)) +
  geom_line(aes(colour = as.factor(Viability)), show.legend = F) +
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = ElapMin, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("C - PCC6803"~"("~2.3~ µm~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
GamSize_CCMP495 <- GamSize_Pred %>%
  dplyr::filter(strain == "CCMP495") %>%
  ggplot(aes(ElapMin, fit)) +
  geom_line(aes(colour = as.factor(Viability)), show.legend = F) +
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = ElapMin, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("D - CCMP495"~"("~3.9~ µm~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
GamSize_CCMP1335 <- GamSize_Pred %>%
  dplyr::filter(strain == "CCMP1335") %>%
  ggplot(aes(ElapMin, fit)) +
  geom_line(aes(colour = as.factor(Viability)), show.legend = F) +
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = ElapMin, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("E - CCMP1335"~"("~6.0~ µm~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
GamSize_CCMP1336 <- GamSize_Pred %>%
  dplyr::filter(strain == "CCMP1336") %>%
  ggplot(aes(ElapMin, fit)) +
  geom_line(aes(colour = as.factor(Viability)), show.legend = F) +
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = ElapMin, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("F - CCMP1336"~"("~16.3~ µm~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
LayoutNiche <- rbind(c(1, 1, 1, 2, 2, 2, 3, 3, 3), 
                     c(1, 1, 1, 2, 2, 2, 3, 3, 3),
                     c(1, 1, 1, 2, 2, 2, 3, 3, 3),
                     c(4, 4, 4, 5, 5, 5, 6, 6, 6),
                     c(4, 4, 4, 5, 5, 5, 6, 6, 6),
...
Supplement_GamSize <- grid.arrange(GamSize_CCMP836 + theme(legend.position = 'none'), GamSize_CZS25K + theme(legend.position = 'none'), GamSize_PCC6803 + theme(legend.position = 'none'), 
                                   GamSize_CCMP495, GamSize_CCMP1335, GamSize_CCMP1336, 
                                   layout_matrix = LayoutNiche)

ggsave(filename = file.path(paste(FP1, FP1, PlotFolder, sep = "/"), paste(Project, "Supplement", "GAM", "SizePlot.png", sep = "_"), fsep = .Platform$file.sep), plot = Supplement_GamSize, height = 22.5, width = 17.8, units = "cm", dpi = 900)

ggsave(filename = file.path(paste(FP1, FP1, PlotFolder, sep = "/"), paste(Project, "Supplement", "GAM", "SizePlot.tiff", sep = "_"), fsep = .Platform$file.sep), plot = Supplement_GamSize, height = 22.5, width = 17.8, units = "cm", dpi = 900)



```

``` {r GAM cell size over time}
E_days_pred <- expand_grid(
  E_days = seq(from=min(0), 
              to=max(7), 
              length.out = 8)
...
GamSize <- SinkSize %>%
  group_by(Viability, par_ue, strain, well) %>% 
  mutate(E_days = as.numeric((datetime - datetime[1])/86400)
         #PhytoGroup = case_when(strain %in% c("CZS25K","PCC6803","CCMP836") ~ "Cyanobacteria",
  #                              strain %in% c("CCMP495","CCMP1335","CCMP1336") ~ "Diatoms")
  ) %>%
  group_by(Viability, par_ue, strain) %>%
  nest() %>%
  mutate(
    GamModel = map(data, ~possibGAM(AvCellArea ~ s(E_days, k = 4), data = .x, 
                                         family = "gaussian"(link = "log"),
                                         method = "REML")),
    predicted = map(GamModel, ~possibPRED(.x, newdata = E_days_pred, type = "response", se.fit = T)),
    augmented = map(GamModel, ~augment(.x)),
    glance = map(GamModel, ~glance(.x))
  )
...
GamSize_Pred <- GamSize %>%
  ...
  dplyr::select(-predicted) %>%
  unnest(col = db) %>%
  full_join(SinkSize %>% group_by(Viability, par_ue, strain, well) %>% mutate(E_days = round(as.numeric((datetime - datetime[1])/86400)))) %>%
  dplyr::filter(Viability != "TotCy5Cells",
                Viability != "DeadAliveCells")  %>%
  mutate(low = fit-2*se.fit,
         high = fit+2*se.fit,
...
GamSize_CCMP836 <- GamSize_Pred %>%
  dplyr::filter(strain == "CCMP836") %>%
  ggplot(aes(E_days, fit)) +
  geom_line(aes(colour = as.factor(Viability)), show.legend = F) +
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = E_days, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("A - CCMP836"~"("~0.9~ µm^3~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
GamSize_CZS25K <- GamSize_Pred %>%
  ...
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = E_days, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("B - CZS25K"~"("~1.8~ µm^3~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
GamSize_PCC6803 <- GamSize_Pred %>%
  ...
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = E_days, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("C - PCC6803"~"("~6.4~ µm^3~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
GamSize_CCMP495 <- GamSize_Pred %>%
  ...
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = E_days, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("D - CCMP495"~"("~31~ µm^3~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
GamSize_CCMP1335 <- GamSize_Pred %>%
  ...
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = E_days, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("E - CCMP1335"~"("~119.5~ µm^3~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
GamSize_CCMP1336 <- GamSize_Pred %>%
  ...
  geom_ribbon(aes(ymin = low, ymax = high, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.3, linetype = 2, show.legend = T) +
  geom_point(aes(x = E_days, y = AvCellArea, color = as.factor(Viability), fill = as.factor(Viability)), alpha = 0.05, show.legend = F) +
  labs(title = bquote("F - CCMP1336"~"("~2950~ µm^3~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
  scale_color_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) +
...
``` {r Boxplots AvCellSizes}
SinkSize %>%
  dplyr::filter(Viability == "AliveCells") %>%
  ggplot(aes(x = as.factor(date_sink), y = log10(AvCellArea))) +
...
SinkSize_CCMP836 <- SinkSize %>%
  dplyr::filter(strain == "CCMP495") %>%
  dplyr::filter(Viability != "TotCy5Cells",
                Viability != "DeadAliveCells",
                Viability != "CorrDeadCells")  %>%
  mutate(par_ue_f = as.factor(par_ue)) %>%
  ggplot(aes(x = as.factor(date_sink), y = (AvCellArea))) +
  geom_boxplot(aes(colour = as.factor(Viability)), show.legend = T) +
  labs(title = bquote("A - CCMP836"~"("~0.9~ µm^3~")")) + 
  scale_y_continuous(name = "Average Cell Area (pixel)",
                     sec.axis = sec_axis(~., name = "Photon flux rate (µE)", breaks = NULL, labels = NULL)) +
  scale_fill_manual(legend_title, values = SizeColors, labels = c("Alive", "Dead")) + 
