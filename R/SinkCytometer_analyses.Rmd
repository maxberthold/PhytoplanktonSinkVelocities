...
SinkReplCCAv <- ImportSink %>%
  group_by(strain, date_sink,datetime, par_ue, ElapMin, TimeID) %>%
  mutate(ReplicateNo = cur_group_id(), 
         cellcount = replace(cellcount, is.na(cellcount), 0) # replace NA with 0, as nothing was detected
         ) %>%
  group_by(ReplicateNo, strain, date_sink, datetime, par_ue, ElapMin, TimeID) %>%
  summarise(cellcount_repl = mean(cellcount, na.rm = TRUE)) 

SinkReplAv <- full_join(SinkReplCCAv, SinkReplTotAreaAv) 
...
SinkAvShort <- MetaData %>%
  ...
  full_join(SinkReplAv) %>%
  distinct() %>%
  group_by(strain, date_sink, par_ue) %>%
  mutate(CCAv = rollmean(cellcount_repl, k = 3, fill = NA), #cell count average, k = 3 as moderate smoother, compared to too noisy or to too much filtering
         TCAreaAv = rollmean(TotalCellArea_repl, k = 3, fill = NA), #total cell area average smooth, k = 3
  ...
  filter(date_sink != "2022-10-20") %>% #data for 20th October currently lost, filter for empty entry
  drop_na(project_id) %>%
...
Diatom <- c("CCMP495")#, "CCMP1335", "CCMP1336")
...
CyanoSinkAll <- Brk_Compare %>%
  ...
  ggplot() +
  geom_point(aes(y = (!!sym(Y_var)), x = ElapMinAv, color = as.factor(date_sink))) +
