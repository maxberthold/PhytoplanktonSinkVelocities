...
SinkMetaAll <- inner_join(SinkTidy, SinkProt, by = c("well_target", "date_sink", "strain")) %>% 
  ...
  mutate(ElapSec = TimeID * 10*60,
         ElapMin = TimeID *10,
         Elapsed.Time.m = seconds_to_period(ElapSec),
         test = as.character(Elapsed.Time.m),
         test2 = format(as.POSIXct(test, format="%HH %MM %SS"), "%H:%M::%S"))
...
SinkMetaAll2 %>%
  ...
  ggplot(aes(x = ElapMin, y = (cellcount), color = as.factor(datetime))) +
  geom_point() +
