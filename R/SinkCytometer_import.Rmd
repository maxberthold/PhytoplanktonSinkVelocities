...
SinkMetaAll <- inner_join(SinkTidy, SinkProt, by = c("well_target", "date_sink", "strain")) %>% 
  ...
  distinct() %>%
  mutate(ElapMin = (TimeID * 10)-5, #TimeIDs are 10 min apart and ID 1 is the very first measurement (0 min), but it took ~2 - 4 minutes to load each plate prior analyses, though *10 - 5 corrects for loadings, but includes the 10 min interval
         ElapSec = ElapMin * 60,
         ElapHour = ElapMin / 60, 
